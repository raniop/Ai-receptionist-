...
const SYNONYMS: Record<string, string[]> = {
  // Non-travel words → the transfer entries (car / home / business).
  מכונית: ["רכב"],
  אוטו: ["רכב"],
  דירה: ["בית"],
  חנות: ["עסק", "עסקים"],
  חברה: ["עסק", "עסקים"],
  // Travel vocabulary → the policy knowledge base.
  חופשה: ["נסיעה", "נסיעות"],
  טיול: ["נסיעה", "נסיעות"],
  אמריקה: ["ארצות", "הברית"],
  ארהב: ["ארצות", "הברית"],
...
export function welcomeMessage(ctx: AssistantContext): AssistantReply {
  ...
  return {
    text: `שלום, הגעתם לאופיר ביטוח, מומחי ביטוחי הנסיעות לחו״ל. אוכל לעזור בהצעת מחיר לנסיעה, שאלות על הכיסוי, תביעות וקביעת פגישות. במה תרצו להתחיל?`,
    actions: actions(ctx),
  };
}
...
export function buildReply(message: string, kb: KbEntry[], ctx: AssistantContext): AssistantReply {
  ...
  if (PRICE.test(text) || hasAny(text, HE.price)) {
    return {
      text: `מחיר ביטוח הנסיעות נקבע לפי גיל הנוסע, היעד ומשך הנסיעה, וארה״ב יקרה יותר משאר היעדים. לנוסע עד גיל חמישים ליעד רגיל המחיר מתחיל בסביבות שני דולר וחצי ליום. אשמח לקחת את הפרטים ולהכין הצעה מדויקת.`,
  ...
  if (CLAIM.test(text) || hasAny(text, HE.claim)) {
    ...
    return {
      text: entry
        ? entry.answer
        : `במקרה חירום רפואי בחו״ל פנו מיד למוקד החירום של הראל, זמין עשרים וארבע שעות ביממה, בטלפון אפס שלוש, שבע, חמש, ארבע, שבע, אפס, שלוש, אפס. אני יכולה גם להסביר איך מגישים תביעה או לרשום את פרטיכם.`,
      actions: [
